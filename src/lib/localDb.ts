...
function isPersistableCourse(course: Partial<LocalCourseRecord> | null | undefined) {
  ...
  const userExists = readLocalUsers().some((user) => user.id === instructorId);
...
export function persistInstructorCourse(course: LocalCourseRecord, instructorId: string) {
  if (!isPersistableCourse({ ...course, instructorId })) {
    throw new Error('Course validation failed: missing instructor or required course data.');
  }

  const users = readLocalUsers();
  const hasInstructor = users.some((user) => user.id === instructorId);
  ...
  const nextUsers: LocalUserRecord[] = hasInstructor
    ? users.map((user) =>
        user.id === instructorId
          ? {
              ...user,
              courseIds: Array.from(new Set([...(user.courseIds ?? []), course.id])),
            }
          : user,
      )
    : [
  ...
  const nextCourses: LocalCourseRecord[] = [
    { ...course, instructorId },
    ...existingCourses.filter((current) => current.id !== course.id),
  ];
...
export function readLocalSession() {
  ...
  try {
    const value = window.localStorage.getItem(getLocalSessionStoreKey());
...
export function writeLocalSession(session: { userId: string; email: string } | null) {
  ...
  window.localStorage.setItem(getLocalSessionStoreKey(), JSON.stringify(session));
}